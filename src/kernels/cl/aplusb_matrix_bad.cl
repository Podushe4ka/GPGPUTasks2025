                     ...
                     unsigned int height)
{
    ...
    const unsigned int index1D = get_global_id(0);
    const unsigned int index2D = get_global_id(1);
    const unsigned int index = index1D * height + index2D;
    if (index1D >= width || index2D >= height)
        return;
